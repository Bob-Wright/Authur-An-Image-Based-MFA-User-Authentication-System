...
<div class="row col-12">
    <nav title="jump to the mugshot gallery" class="btn" style="background-color: orange">
		...
		</g><a xmlns="http://www.w3.org/2000/svg" id="anchor" xlink:href="./Demo.php" xmlns:xlink="http://www.w3.org/1999/xlink" target="_top"><rect x="0" y="0" width="100%" height="100%" fill-opacity="0"/></a>
...
if(getenv('QUERY_STRING') != '') {
	...
	// see if valid URL query string keys
		if(array_key_exists('mug', $pstrings)) { // user image filename
			$mug = $pstrings['mug'];
			$_SESSION["ckFileName"] = $mug;
		}
		if(array_key_exists('dice', $pstrings)) { // dice triplet value
			$dice = $pstrings['dice'];
			$_SESSION["dice"] = $dice;
		}
}
...
$mugIndex = substr($_SESSION["ckFileName"], 2);
// find our chosen image
$imageName = $ckNames[$mugIndex - 1];

			echo
			'<hr>'.
			'<h2 class="col-12">The gatekeeper you chose is named "'.$imageName.'".</h2>'.
			'<h2 class="col-12">The gatekeeper\'s dice are "'.$dice.'".</h2>'.
			'<div class="row d-flex justify-content-center">'.
				'<h2 class="col-12">This is the mugshot image for '.$imageName.'. Click the image to continue.</h2>'.
				'<form action="./Demo.php">'.
					'<input type="image" class="col-8" src="./ckimagesPNG/'.$mug.'.png" width="100%" height="auto" alt="'.$imageName.'">'.
				'</form>'.
			'</div>'.
