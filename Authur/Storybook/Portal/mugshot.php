...
if (($_SERVER["REQUEST_METHOD"] == "POST") && (isset($_POST['userEmail'])) && ($_POST['userEmail'] != '')) {
	$userEmail = $_POST['userEmail'];
	$_SESSION['userEmail'] = $userEmail;}

// extract values from imageName POST string
// value="tile=1&mug='.$displayFiles[0].'&token='.$diceOrder[0].'&dice=' . $diceImages[0][0] . $diceImages[0][1] . $diceImages[0][2] . '"
$mugshotData = explode('&', $imageName);
$mugshot = explode("=", $mugshotData[1]);
$mugshot = $mugshot[1];
$mugshotFile = $mugshot;
$_SESSION['mugshotFile'] = $mugshotFile; // image filename
$mugshot = substr($mugshot, 2);
$_SESSION['mugshot'] = $mugshot; // numeric part of image filename
$triplet = explode("=", $mugshotData[3]);
$triplet = $triplet[1];
$_SESSION['triplet'] = $triplet; // value of the spots on the dice
...
/*	if($is_active == 0) { 
		$update = mysqli_query($connection, "UPDATE users SET is_active = '1' WHERE email LIKE '$userEmail'");
		if(!($update)) {
		$_SESSION['email_error'] = '<div class="alert alert-danger">
		email match error.</div>';
		die($_SESSION['email_error']);
			...
			'<div class="row d-flex justify-content-center">'.
				'<h2 class="col-12">This is the mugshot image for '.$mugshotName.'. Click the image to continue.</h2>'.
