...
// Key Database connection get user public key
//user public key as plaintext to encode within our image
include('/var/www/Storybook/htdocs/config/db.php');
$sqlQuery = mysqli_query($connection, "SELECT pubkey FROM pubkey WHERE email LIKE '$userEmail'");
$countRow = mysqli_num_rows($sqlQuery);
$_SESSION['countrow'] = $countRow;
if($countRow <= 0){ //see if we have a key
	$_SESSION['key_error'] = '<div class="alert alert-danger">
		email key match error.</div>';
	die($_SESSION['key_error']);} 
$rowData = mysqli_fetch_row($sqlQuery);
 $_SESSION['plaintext'] = $rowData[0];
if(!($rowData)) {
		$_SESSION['key_error'] = '<div class="alert alert-danger">
		key read error.</div>';
		die($_SESSION['key_error']);} 
//$plaintext = 'plaintext';
$plaintext = $_SESSION['plaintext']; //user public key as plaintext to encode within our image

// get $coverImage jpg
$coverImage = './ckimages/'. $_SESSION['mugshotFile'] .'.jpg';
...
list($width, $height, $type, $attr) = getimagesize($coverImage);
...
// second parameter not necessary if result put in stream 
$stegoContainer->encode($coverImage, '', $plaintext);
...
//if(is_file('./testStego.png')) {
	//unlink('./testStego.png');}
$file = fopen($stegoImage, "w") or die("Unable to open file!");
fwrite($file, $page);
fclose($file);
//echo $page;
			echo
			'<hr>'.
			'<h2>Your User Account Creation Was Successfull!</h2>'.
			'<h3>You may now log in to the Storybook application.</h3><br>'.
			// '<h2 class="col-12">The gatekeeper you chose is named "'.$mugshotName.'".</h2>'.
			'<h2 class="col-12">Your email is "'.$_SESSION['userEmail'].'".</h2>'.
			'<div class="row d-flex justify-content-center">'.
				'<h2 class="col-12">This is the mugshot image for the gatekeeper avatar named "'.$_SESSION['mugshotName'].'".<br>Click the image to complete your registration.</h2>'.
				'<form action="../controllers/logout.php">'.
					'<input type="image" class="col-8" src="./testStego.png" width="100%" height="auto" alt="'.$_SESSION['mugshotName'].'">'.
				'</form>'.
			'</div>'.
